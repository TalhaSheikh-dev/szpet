...
from src.scorer import Scorer, Writer
from src.szpet import adapet
...
def eval(config, model, batch_iter, scorer):
    ...
    model.eval()
    with torch.no_grad():
        for idx, batch in tqdm(enumerate(batch_iter), desc='Eval Batches'):
...
def load_model(config, model_path):
    tokenizer = transformers.AutoTokenizer.from_pretrained(config.pretrained_weight)
