...
class adapet(torch.nn.Module):
    def __init__(self, config, tokenizer, dataset_reader):
        ...
        if "albert" in pretrained_file:
            albert_config = tfrms.AlbertConfig.from_pretrained(pretrained_file)
            self.model = tfrms.AlbertForMaskedLM.from_pretrained(pretrained_file, config=albert_config)
        else:
            self.model = tfrms.AutoModelForMaskedLM.from_pretrained(pretrained_file)
        ...
        self.pattern_list = self.dataset_reader.pets
    ...
    def get_eval_multilbl_logits(self, pet_mask_ids, batch_mask_idx, batch_list_lbl):
        ...
        for idx, lbl in enumerate(list_lbl):
            ...
            while True:
                ...
                pet_rep = self.model(pet_mask_ids[idx:idx + 1], (pet_mask_ids[idx:idx + 1] > 0).long())[0]  # [bs, max_seq_len]
                next_token_logits = pet_rep.softmax(dim=-1)[0]  # The last indexing operation gets rid of batch dimension
